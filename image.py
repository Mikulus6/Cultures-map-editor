...
def bytes_to_image(sequence: bytes, filename: str, width: int):
    image = Image.new('L', (width, len(sequence)//width))
    image.putdata(sequence)
    image.save(filename)


def shorts_to_image(sequence: bytes, filename: str, width: int):
...
def bits_to_image(sequence: str, filename: str, width: int):
