...
pattern_edit_group_path = "data_v\\ve_edit\\patternrandomeditgroups.cif"
pattern_edit_group = load_ini_as_dict(pattern_edit_group_path,
                                      allowed_section_names=("patterngroup",),
                                      entries_duplicated=("Pattern",),
                                      global_key=lambda x: x["Name"],
                                      merge_duplicates=False)

def get_random_group_entry(group_elements, *, legacy_randomness: bool = False):
    values_sum = sum(entry[1] for entry in group_elements)
